...
class ConfigParser(configobj.ConfigObj):
    ...
    def __init__(self, config_file, indent_type="  ", encoding="utf-8"):
        ...
        self.logger.info("Found plugins: {0}".format(", ".join(list(self.plugins.keys()))))
        ...
        for plugin in self.keys():
            if plugin != "Version" and plugin != "Categories":
                ...
                if plugin_info["data_type"] == "string":
                    self.logger.info("Checking string rules for plugin {0}".format(plugin))
                ...
                if category:
                    if category in self.categories:
                        ...
                        self.logger.debug("Adding {0} to category {1}".format(plugin, category))
                    else:
                        self.logger.error(
                            "{0} have an undefined category {1}".format(plugin, category)
                        )
                        self.logger.info(
        ...
        for score_name in score_dict:
            ...
            if score_name == "not_reported":
                ...
                if raw_info["score"] == "eq":
                    ...
                    score_function.add_value(value=raw_info["value"], score=raw_info["score"])
                else:
                    lower_bound = float(raw_info["lower"])
                    upper_bound = float(raw_info["upper"])
                    score = float(raw_info["score"])
                    score_function.add_interval(lower=lower_bound, upper=upper_bound, score=score)
...
@click.option("-out", "--outfile", nargs=1, type=click.File("w"))
@click.option("-l", "--loglevel", type=click.Choice(["DEBUG", "INFO", "WARNING"]), default="INFO")
def read_config(config_file, outfile, loglevel):
    ...
    for plugin in config_reader.plugins:
        ...
        logger.info("{0}: {1}".format(plugin, config_reader.plugins[plugin]))

    for category in config_reader.categories:
        logger.info("Category {0}: {1}".format(category, config_reader.categories[category]))
