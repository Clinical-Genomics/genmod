    ...
    for variant_id in variant_batch:
        ...
        for ind_id in individuals:
            ...
            if "|" in raw_gt_call:
                ...
                logger.debug("Variant {0} is phased for individual {1}.".format(variant_id, ind_id))
                ...
                if not in_haploblock[ind_id]:
                    ...
                    logger.debug("Setting is_haploblock to True for individual {0}".format(ind_id))
            ...
            # Here the variant is not phased
                ...
                if variant.get("FILTER", ".") == "PASS":
                    ...
                    if in_haploblock[ind_id]:
                        logger.debug(
                            "Creating a haploblock for individual {0}"
                        ...
                        haploblocks[ind_id].append(
                            [haploblock_starts[ind_id], int(variant["POS"]) - 1, str(haploblock_id)]
                        )
                        haploblock_id += 1
                        logger.debug("Setting haploblock id to {0}".format(haploblock_id))
                        in_haploblock[ind_id] = False
                        logger.debug(
                            "Setting is_haploblock to False for individual {0}".format(ind_id)
                        )
    ...
    for ind_id in individuals:
        ...
        if in_haploblock[ind_id]:
            logger.debug(
                "Creating a haploblock for individual {0}"
                " with start:{1}, stop:{2} and id:{3}".format(
                    ind_id, haploblock_starts[ind_id], int(variant["POS"]) - 1, str(haploblock_id)
                )
            )
            haploblocks[ind_id].append(
                [haploblock_starts[ind_id], int(variant["POS"]), str(haploblock_id)]
            )
        ...
        if haploblocks[ind_id]:
            logger.debug(
                "Creating IntervalTree for individual {0} with haploblocks:{1}, "
            ...
            interval_trees[ind_id] = IntervalTree(
                haploblocks[ind_id], haploblocks[ind_id][0][0] - 1, haploblocks[ind_id][-1][1] + 1
            )
