...
class ConfigObjWithNamedTemporaryFile(ConfigObj):
    ...
    def add_category(self, name: str, aggregation_mode: str):
        """
...
def config_file() -> ConfigObjWithNamedTemporaryFile:
    ...
    config: ConfigObjWithNamedTemporaryFile = ConfigObjWithNamedTemporaryFile(
...
@pytest.fixture
def variant_0():
    variant = {"CHROM": "1", "POS": "1", "INFO": "plugin_a=0.01;plugin_b=5.0"}
    variant = set_info_dict_in_variant(variant)
    return variant


@pytest.mark.parametrize(
    "aggregation_mode,expected", [("sum", (2, 0, 2)), ("max", (2, 0, 2)), ("min", (2, 0, 2))]
)
def test_single_category_single_plugin(config_file, variant_0, aggregation_mode, expected):
    ...
    config_file.add_plugin(
        "CategoryA", "PluginA", {"rare": {"score": 2.0, "lower": 0.01, "upper": 0.05}}, "plugin_a"
    )
    config_parser: ConfigParser = ConfigParser(config_file.filename)
    # WHEN scoring a variant
    score, min, max = get_category_score(
        variant=variant_0, category="CategoryA", config_parser=config_parser
    )
...
@pytest.mark.parametrize(
    "aggregation_mode,expected", [("sum", (2, -5, 2)), ("max", (2, -5, 2)), ("min", (2, -5, 2))]
)
def test_single_category_single_plugin_not_reported(
...
@pytest.mark.parametrize(
    "aggregation_mode,expected", [("sum", (3, 0, 3)), ("max", (2, 0, 2)), ("min", (1, 0, 1))]
)
def test_single_category_two_plugin(config_file, variant_0, aggregation_mode, expected):
    ...
    config_file.add_plugin(
        "CategoryA", "PluginB", {"rare": {"score": 1.0, "lower": 4.0, "upper": 6.0}}, "plugin_b"
    )
    config_parser: ConfigParser = ConfigParser(config_file.filename)
    # WHEN scoring a variant
    score, min, max = get_category_score(
...
def test_single_category_two_plugin_negative_score_range(config_file, variant_0, aggregation_mode, expected):
    ...
    config_file.add_category("CategoryA", aggregation_mode)
    config_file.add_plugin(
        "CategoryA", "PluginA", {"rare": {"score": 2, "lower": 0.01, "upper": 0.05}}, "plugin_a"
    )
    config_file.add_plugin(
        "CategoryA", "PluginB", {"rare": {"score": -5, "lower": 4.0, "upper": 6.0}}, "plugin_b"
    )
    config_parser: ConfigParser = ConfigParser(config_file.filename)
    # WHEN scoring a variant
    score, min, max = get_category_score(
...
def test_multi_category_two_plugins(config_file, variant_0, aggregation_mode, expected):
    ...
    config_file.add_plugin(
        "CategoryB", "PluginB", {"rare": {"score": 1.0, "lower": 4.0, "upper": 6.0}}, "plugin_b"
    )
    config_parser: ConfigParser = ConfigParser(config_file.filename)
...
@pytest.mark.parametrize(
    "aggregation_mode,expected", [("sum", (10, 0, 10)), ("max", (10, 0, 10)), ("min", (10, 0, 10))]
)
def test_single_category_single_plugin_multirange(
    config_file, variant_0, aggregation_mode, expected
):
    """
