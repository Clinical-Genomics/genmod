...
from genmod.vcf_tools import HeaderParser, get_variant_dict, get_info_dict, get_variant_id
...
def test_rescore_with_annotation_suffix():
    ...
    assert len(original_variants) == len(rescored_variants), \
        (len(original_variants), len(rescored_variants))

    unaltered_annotations = ['RankScore',
                             'RankScoreNormalized',
                             'RankScoreMinMax',
                             'Compounds']
    ...
    for unalted_annotation in unaltered_annotations:
        for original_variant, rescored_variant in zip(original_variants, rescored_variants):
            ...
            if unalted_annotation == 'Compounds' and \
                unalted_annotation not in original_variant.keys():
                continue
            assert \
            original_variant[unalted_annotation] == rescored_variant[unalted_annotation], \
    ...
    for added_annotation in added_annotations:
        for original_variant, rescored_variant in zip(original_variants, rescored_variants):
            ...
            if (
                ...
                continue
            assert added_annotation not in original_variant.keys(), (added_annotation, original_variant)
